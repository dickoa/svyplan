...
Domain columns are specified explicitly via the `domains` parameter.
...
Given a sampling frame with stratum population sizes (`N_h`) and stratum standard deviations (`S_h`), `n_alloc()` distributes a total sample across strata. This is the second half of a stratified design: `strata_bound()` finds the cut points, and `n_alloc()` allocates within strata.
...
Three solve modes are available: fixed total `n`, target `cv`, or `budget` constraint (when `cost_h` is provided in the frame).
