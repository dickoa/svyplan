...
The standard cost model is purely linear: `C = c1*n1 + c2*n1*n2`. Real surveys also have a fixed overhead (training, infrastructure, setup). The `fixed_cost` parameter adds this term: `C = C0 + c1*n1 + c2*n1*n2`. In budget mode, only `budget - fixed_cost` is available for the variable component, reducing n1 while leaving the optimal n2/n1 ratio unchanged. In CV mode, all sample sizes stay the same and `fixed_cost` is simply added to the total cost:
...
- `"power"`: Bankier (1988) compromise, $n_h \propto S_h \cdot N_h^q$,
  where $q \in [0, 1]$ controls the trade-off between national precision
  ($q = 1$, Neyman) and equal subnational CVs ($q = 0$)

```{r strata-power-alloc}
# Bankier power allocation (compromise, q = 0.5)
strata_bound(x, n_strata = 4, n = 300, alloc = "power", q = 0.5)
```
