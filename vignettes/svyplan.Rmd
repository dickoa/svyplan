...
Every function returns an S3 object with `print`, `predict`, and (where applicable) `confint` methods.
...
Three solve modes are available: fixed total `n`, target `cv`, or `budget` constraint (when `cost` is provided in the frame).
...
7. Optimize the multistage allocation with `n_cluster()` or `n_multi()` with costs
